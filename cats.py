...
def accuracy(typed, reference):
    ...
    if reference_words == [] and typed_words != [] or typed_words == [] and reference_words != []:
        return 0.0
...
def fastest_words(game):
    ...
    for word in word_indices:
        ...
        i = 0
        for player in player_indices:
            scores_fo_dis_word.append((time(game, player, word)))
        for score in scores_fo_dis_word:
            if score == min(scores_fo_dis_word):
                scores[i].append(word_at(game, word))
                break
            i += 1
    return scores
